# -*- coding: utf-8 -*-
from .sql import Sql
from hello.items import HelloItem

class HelloPipeline(object):

    def process_item(self, item, spider):
        # deferToThread(self._process_item, item, spider)
        if isinstance(item, HelloItem):
            name_id = item['name_id']
            ret = Sql.select_name(name_id)
            if ret[0] == 1:
                print '已经存在了'
                pass
            else:
                xs_name = item['name']
                xs_author = item['author']
                category = item['category']
                Sql.insert_dd_name(xs_name, xs_author, category, name_id)
                print '开始存小说标题'

        if isinstance(item, spider):    # 可将判断移动到Spider中，减少一次Request，减轻服务器压力
            url = item['chapterurl']
            name_id = item['id_name']
            num_id = item['num']
            xs_chatername = item['chaptername']
            xs_content = item['chaptercontent']
            Sql.insert_dd_name(xs_chatername, xs_content, name_id, num_id, url)
            print '小说存储完毕'
            return item
